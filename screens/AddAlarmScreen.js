  ...
  const handleSaveAndBack = async () => {
    ...
    try {
      ...
      if (isActive) {
        ...
        if (notificationId) {
          const alarmRef = firestore().collection('alarms').doc(alarmId);
          await alarmRef.update({ notificationId });
