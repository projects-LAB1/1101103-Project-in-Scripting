...
const SoundPickerScreen = ({ route, navigation }) => {
  const { selectedSoundId, onSelectSound } = route.params || {};
  const [sound, setSound] = useState(null);
  const [playingId, setPlayingId] = useState(null);
  const [selectedId, setSelectedId] = useState(selectedSoundId || 'default');
  ...
  const handleSave = () => {
    const selectedSound = availableSounds.find(item => item.id === selectedId);
